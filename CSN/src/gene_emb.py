...
def main(argv):
    ...
    def write_cascade(graphs, labels, filename, s_weight,print_note='Write cascade graphs', weight=True):
        """
        ...
        for key, graph in graphs.items():
            ...
            if s_weight=='social':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(node))
                    cascade_embedding.append(emb) 
            elif s_weight=='inter':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],i_emb.get(node))
                    cascade_embedding.append(emb)
            else:
                for node in nodes_index:
    ...
    def write_aug_cascade(graphs, filename,s_weight, print_note='augmentation 1', weight=True):
        """
        Input: cascade graphs, global embeddings
        Output: cascade embeddings, with global embeddings appended
        """
    ...
    f_read = open('/home/jpf/swj/work2/ccgl_r/datasets/weibo/inter_rwpe.pkl', 'rb')
    i_emb = pickle.load(f_read)
    f_read.close()
    f_read = open('/home/jpf/swj/work2/ccgl_r/datasets/weibo/social_rwpe.pkl', 'rb')
    s_emb = pickle.load(f_read)
    f_read.close()
    ...
    graphs_aug_1 = sequence2list(FLAGS.input + 'train.txt')
    graphs_aug_2 = sequence2list(FLAGS.input + 'train.txt')
    ...
    write_cascade(graphs_train, label_train, FLAGS.input + 'train.pkl','social', print_note='(1/7) Write train set')
    print("Start writing validation set into file.")
    write_cascade(graphs_val, label_val, FLAGS.input + 'val.pkl','social', print_note='(2/7) Write val set')
    print("Start writing test set into file.")
    write_cascade(graphs_test, label_test, FLAGS.input + 'test.pkl', 'social',print_note='(3/7) Write test set')
    print("Start writing aug set 1 into file.")
    write_aug_cascade(graphs_aug_1, FLAGS.input + 'data_aug_1.pkl', 'social',print_note='(4/7) Write augmented cascade graphs')
    print("Start writing aug set 2 into file.")
    write_aug_cascade(graphs_aug_2, FLAGS.input + 'data_aug_2.pkl', 'inter',print_note='(5/7) Write augmented cascade graphs')
