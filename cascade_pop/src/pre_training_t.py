        ...
        for i in range(len(aug_1) // batch_size):
            ...
            for batch_cascade_1 in batch_aug_1:
                while len(batch_cascade_1) < max_seq:
                    batch_cascade_1.append(np.zeros(emb_dim+1))
            for batch_cascade_2 in batch_aug_2:
                while len(batch_cascade_2) < max_seq:
                    batch_cascade_2.append(np.zeros(emb_dim+1))
            ...
            batch_aug_1 = torch.from_numpy(np.delete(np.array(batch_aug_1),64,2)).float().to(device)
            batch_aug_2 = torch.from_numpy(np.delete(np.array(batch_aug_2),64,2)).float().to(device)            
