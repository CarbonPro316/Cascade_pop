...
def main(argv):
        ...
        for key, graph in graphs.items():
            ...
            if s_weight=='weibo':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/2000000)
                    cascade_embedding.append(emb)
            elif s_weight=='acm':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/100000)
                    cascade_embedding.append(emb)
            elif s_weight=='aps':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/300000)
                    cascade_embedding.append(emb)
            elif s_weight=='dblp':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/100000)
                    cascade_embedding.append(emb)
            elif s_weight=='twitter':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/300000)
                    cascade_embedding.append(emb)
            else:
    ...
    def write_aug_cascade(graphs, filename,s_weight, print_note='augmentation 1', weight=True):
        ...
        for key, graph in graphs.items():
            ...
            if s_weight=='weibo':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/200000)
                    cascade_embedding.append(emb) 
            elif s_weight=='aps':
                for node in nodes_index:
                    emb=np.append(chi[nodes_index_unique.index(node)],s_emb.get(str(node),1)/300000)
    ...
    s_emb={}
    if s_weight=='weibo':
        f_read = open('/home/jpf/swj/ccgl_pytorch/datasets/weibo/graph.pkl', 'rb')
        s_emb = pickle.load(f_read)
        f_read.close()
    elif s_weight=='acm':
        f_read = open('/home/jpf/swj/ccgl_pytorch/datasets/acm/graph.pkl', 'rb')
        s_emb = pickle.load(f_read)
        f_read.close()
    elif s_weight=='dblp':
        f_read = open('/home/jpf/swj/ccgl_pytorch/datasets/dblp/graph.pkl', 'rb')
        s_emb = pickle.load(f_read)
        f_read.close()
    elif s_weight=='twitter':
        f_read = open('/home/jpf/swj/ccgl_pytorch/datasets/twitter/graph.pkl', 'rb')
        s_emb = pickle.load(f_read)
        f_read.close()
    elif s_weight=='aps':
        f_read = open('/home/jpf/swj/ccgl_pytorch/datasets/aps/graph.pkl', 'rb')
        s_emb = pickle.load(f_read)
        f_read.close()
    else:
    ...
    #write_aug_cascade(graphs_aug_1, FLAGS.input + 'data_aug_1.pkl', s_weight,print_note='(4/7) Write augmented cascade graphs')
    #print("Start writing aug set 2 into file.")
    #write_aug_cascade(graphs_aug_2, FLAGS.input + 'data_aug_2.pkl', s_weight,print_note='(5/7) Write augmented cascade graphs')
